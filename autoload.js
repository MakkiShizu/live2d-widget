...
function Live2d() {
  ...
  class Live2d {
    ...
    init(path, config = {}) {
      ...
      if (screen.width >= 768) {
        Promise.all([
          util.loadExternalResource(path + "waifu.css", "css"),
          util.loadExternalResource(path + "live2d.min.js", "js"),
        ]).then(() => {
          this.#path = path;
          this.defaultConfig.tipsPath = path + "waifu-tips.json";
    ...
    #doInit() {
      document.body.insertAdjacentHTML(
        "beforeend",
        `<div id="live2d-toggle"><span>看板娘</span></div>`
      );
      ...
      if (
        localStorage.getItem("live2d-display") &&
        Date.now() - localStorage.getItem("live2d-display") <= 86400000
      ) {
    ...
    #loadWidget() {
      ...
      if (this.#config["isTools"] === true) {
        if (typeof Iconify !== "undefined") {
          ...
          util
            .loadExternalResource(this.#path + "iconify.min.js", "js")
            .then(() => {
              tools._registerTools(model, this.#config);
            });
        }
    ...
    #registerEventListener(result) {
      ...
      if (this.#config["openConsole"] === true) {
        ...
        devtools.toString = () => {
          message.showMessage(
            this.#config["openConsoleTip"] || result["message"]["console"],
            6000,
            2
          );
        };
      ...
      if (this.#config["copyContent"] === true) {
        window.addEventListener("copy", () => {
          message.showMessage(
            this.#config["copyContentTip"] || result["message"]["copy"],
            6000,
            2
          );
        });
      ...
      if (this.#config["backSite"] === true) {
        window.addEventListener("visibilitychange", () => {
          if (!document.hidden) {
            message.showMessage(
              this.#config["backSiteTip"] ||
                result["message"]["visibilitychange"],
              6000,
              2
            );
          }
    ...
    #initModel(model) {
      ...
      if (modelId === null || !!this.#config["isForceUseDefaultConfig"]) {
        ...
        modelTexturesId = this.#config["modelTexturesId"] || 2; // 材质 ID
      }
  ...
  util.mergeTips = function (configTips, themeTips, defaultTips) {
    ...
    let duplicateMouseover = [
      ...configTips["mouseover"],
      ...themeTips["mouseover"],
      ...defaultTips["mouseover"],
    ];
    ...
    return {
      ...
      callback: () => {
        ...
        setTimeout(() => {
          ...
          document
            .getElementById("live2d-toggle")
            .classList.add("live2d-toggle-active");
        }, 3000);
      },
    };
  };
  ...
  tools._registerTools = function (model, config) {
    ...
    for (let tool of config.tools) {
      if (tools[tool]) {
        ...
        document
          .getElementById(`live2d-tool-${tool}`)
